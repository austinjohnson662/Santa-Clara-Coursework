...
typedef union extra_info {
	char name[LENGTH]; //Name of B-Day person
	int years; //Anniversary: How many years together
	int age; //if no occasion enter the average age of the group
}EXINFO;
...
int insertParty(){
	if (counter != SIZE){
		...
		scanf("%d", &occasionInput);
		
		if (check_duplicate(inputName) == 0){		
			strcpy(partyList[counter].name, inputName);
			partyList[counter].size = inputSize;
		
			//if input is 1 asks for name of the birthday. if 2 its an anniversary. Otherwise, input 0 and asks for average age of the group.	
			if (occasionInput== 1){
				partyList[counter].specialOccasion = 1;
				printf("Name: ");
				scanf("%s", inputName);
				strcpy(partyList[counter].info.name, inputName);
			} else if (occasionInput == 2){
				partyList[counter].specialOccasion = 2;
				printf("Years: ");
				scanf("%d", &partyList[counter].info.years);
			} else {
				partyList[counter].specialOccasion = 0;
				printf("Age: ");
				scanf("%d", &partyList[counter].info.years);
			}
			counter++;
...
int list(){
	...
	for (i = 0; i <  counter; i++){
		...
		//checks for occasion to create desired output
		if (p->specialOccasion == 1)
			printf("Occasion: Birthday Name: %s\n", p->info.name);
		else if (p->specialOccasion == 2)
			printf("Occasion: Anniversary Years: %d\n", p->info.years);
		else
			printf("Occasion: none Age: %d\n", p->info.age);
	...
	for (i = 0; i < counter; i++){
		if (p->size <= checkSize){
			...
			if (p->specialOccasion == 1)
				...
				printf("Occasion: Anniversary Age: %d\n", p->info.years);
			else
				printf("Occasion: none Years: %d\n", p->info.age);
