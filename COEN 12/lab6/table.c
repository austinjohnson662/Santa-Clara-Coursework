...
int partition(void *a[], int lo,int hi, int (*compare)()){
	int p;
	p = (lo+hi)/2;
	void *pivot = a[p];
	while(lo <= hi){
...
void quicksort(void **a, int lo, int hi, int (*compare)()){

	if (lo<hi){
		int pi = partition(a,lo,hi,compare);

		quicksort(a,lo, pi-1,compare);
		quicksort(a,pi,hi,compare);
	}
	return;
}
...
void *getElements(SET *sp){
	...
	quicksort(a, 0, sp->count-1, sp->compare);
	return a;
}
